...
3. **הגדרות מותאמות אישית**
   ...
   - הגדרות זמנים:
     * מעבר אוטומטי בין שעון חורף לקיץ
     * בחירת שיטת חישוב עלות השחר (72/90 דקות)
     * בחירת שיטת חישוב צאת הכוכבים (14/22.5/24 דקות)
     * בחירת שיטת חישוב מוצאי שבת (רגיל/חזון איש)
...
- שימוש בפונקציית `render` המשופרת להצגת אירועים בפורמט עברי
...
- הוספת אפשרויות התאמה אישית לזמנים:
  ...
  * **מוצאי שבת**: אפשרות לבחור בין מוצאי שבת רגיל לחזון איש
...
- טיפול בכל סוגי האירועים (אובייקטים ומחרוזות)
- תצוגה משופרת של אירועים בחלון המודאלי
- שיפור נראות האירועים בעת מעבר עכבר מעל תא היום:
  * שינוי צבע טקסט האירועים ללבן
  * הוספת רקע שקוף לאירועים
  * הוספת צל טקסט לשיפור הקריאות
- שיפור תצוגת פרטי היום:
  * הצגת אירועים מסודרת כולל פרשת השבוע
  * תצוגה ברורה של כל האירועים במודאל
...
### שיפורי UI
- הוספת מידע על תכונת שעון הקיץ החדשה
  * מעבר אוטומטי בין שעון חורף לקיץ
  * התאמת כל הזמנים (זריחה, שקיעה, זמני תפילה וכו')
  * אפשרות לבחור בין מעבר אוטומטי או ידני
  * חישוב מדויק של תאריכי המעבר לפי חוקי מדינת ישראל:
    - תחילת שעון קיץ: יום שישי האחרון של מרץ בשעה 2:00
    - סיום שעון קיץ: יום ראשון האחרון של אוקטובר בשעה 2:00
...
## שינויים אחרונים
### תיקוני UI
- הסרת כפתור סגירה כפול במודאל פרטי היום
- שיפור תצוגת התאריך העברי במודאל פרטי היום
  * הסרת תווים מיותרים
  * שימוש בפונקציות מובנות של Hebcal להצגת תאריכים עבריים

### שיפורי אופליין
- עדכון Service Worker לתמיכה מלאה בעבודה ללא חיבור אינטרנט
  * מזעור תלות בקבצים חיצוניים
  * שימוש בספריות מקומיות
  * שיפור מנגנון Cache API
- הסרת תלות בקישורי CDN חיצוניים
  * החלפת קישורי ספריות חיצוניים בעותקים מקומיים
  * הוספת תמיכה מלאה בטעינת סקריפטים גלובליים
- שיפור טיפול בטעינת משאבים באופן מקומי
  * הגדרת אסטרטגיית Cache מדויקת יותר
  * הסרת בקשות מיותרות לשרתים חיצוניים


  שינויים אחרונים
9 בפברואר 2025
filterTimesBySettings
שופר לתמוך בהגדרות מגוונות:
עלות השחר (72 או 90 דקות)
צאת הכוכבים (14, 22.5, ו-24 דקות)
מוצאי שבת (רגיל או חזון)
Service Worker (sw.js)
שיפור טיפול בכישלונות fetch
הוספת מנגנון גיבוי לטעינת דף אינדקס
שיפור טיפול בשגיאות
showDayDetails
עדכון להצגת זמנים מדויקים יותר
תמיכה בהגדרות משתמש מתקדמות

## שיטת שמירת הגדרות

### גישה חדשנית לניהול הגדרות האפליקציה

#### רקע
בגרסאות קודמות של האפליקציה, כל ההגדרות נשמרו באובייקט אחד. גישה זו יצרה קושי בניהול והרחבת ההגדרות.

#### השיטה החדשה
פיתחנו שיטה חדשה לשמירת הגדרות, המאפשרת:
- שמירת הגדרות לכל קטגוריה במפתח נפרד ב-`localStorage`
- גמישות בהוספת והרחבת סוגי הגדרות
- טעינה קלה ואינטואיטיבית של הגדרות

##### מפתחות השמירה העיקריים:
- `display-settings`: הגדרות תצוגה (גודל טקסט, צבע נושא)
- `times-settings`: הגדרות זמנים (סוגי זמנים, הצגת זמנים)
- `settings`: הגדרות כלליות נוספות

#### יתרונות השיטה
- הפרדת תחומי אחריות בין סוגי ההגדרות
- קלות בתחזוקה ובהרחבת הקוד
- שמירה על עקרונות SOLID בפיתוח תוכנה

#### דוגמה לשמירת הגדרות
```javascript
// שמירת הגדרות תצוגה
localStorage.setItem('display-settings', JSON.stringify({
    fontSize: 'medium',
    themeColor: 'blue'
}));

// שמירת הגדרות זמנים
localStorage.setItem('times-settings', JSON.stringify({
    dawnType: '72',
    tzeitType: '14',
    showDawn: true
}));
